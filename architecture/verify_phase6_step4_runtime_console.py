...
def concept_fold(value):
    normalized=unicodedata.normalize('NFKC',str(value or ''));visible=''.join(ch for ch in normalized if unicodedata.category(ch)!='Cf');compact=re.sub(r'\s+',' ',visible).strip();return re.sub(r'[.!?]+$','',compact).strip().casefold()
...
def main():
    ...
    core=read(ROOT/'⚡'/'⚡');preload=read(ROOT/'⚡'/'🔌');auth=read(ROOT/'⚡'/'🔐');admin=read(ROOT/'⚡'/'🛡️');ui=read(ROOT/'⚡'/'🎛️');css=read(ROOT/'⚡'/'🕶️');runtime=read(ROOT/'↔️'/'⚡');vision=read(ROOT/'📸'/'📸')
    ...
    req('Vision parameters' in ui and 'Vision probes' in ui and 'Vision patch size' in ui,'native vision missing from Training Center')
    req('cursor:inherit!important' in css and '#el-admin-entry:hover' in css,'zero-clue CSS missing')
    req('forgey_primary_released' in runtime and 'provider_calls": 0' in runtime and 'SelectedGeneration' in runtime,'Forgey-first text evidence missing')
    ...
    req('Forgey Insta native vision is attempted first' in vision and 'native_forgey_vision_released' in vision and 'OllamaConnector' in vision,'Forgey-first screenshot routing missing')

    step5_manifest_path=ROOT/'architecture'/'phase6_step5_release_manifest.json';publisher_path=ROOT/'scripts'/'publish-step5-main-release.ps1';step5_state='ABSENT'
    if step5_manifest_path.exists():
        step5=load(step5_manifest_path);req(step5.get('phase')==6 and step5.get('step')==5,'invalid later Step5 manifest');req(step5.get('status') in {'IMPLEMENTATION_IN_PROGRESS','PASS'},'invalid later Step5 status');req(step5.get('base_main_sha')=='e0ed1b2ac91ae1f9a716abfc0e93904469b91422','Step5 does not descend from merged Step4');req(publisher_path.exists(),'authorized Step5 manifest exists without Step5-owned publisher');step5_state=str(step5.get('status'))
    else:req(not publisher_path.exists(),'Step5 publisher leaked into Step4 without Step5 authority')

    evidence_dir=ROOT/'data'/'phase6-step4'
    for name in ('primary-forward.json','primary-reverse.json','status.json','auth-proof.json','console-proof.json'):req((evidence_dir/name).is_file(),f'missing Step4 evidence {name}')
    ...
    params=int(s.get('model',{}).get('trainable_parameters') or 0);vision_params=int(s.get('model',{}).get('vision_parameters') or 0)
    req(1000000<=params<=3000000,'runtime parameter count outside derived authority')
    req(s.get('model',{}).get('vision_enabled') is True and vision_params>0,'runtime native vision unavailable')
    req(int(s.get('model',{}).get('vision_image_size') or 0)==64 and int(s.get('model',{}).get('vision_patch_size') or 0)==8 and int(s.get('model',{}).get('visual_tokens') or 0)==64,'runtime native vision geometry mismatch')
    req(s.get('training',{}).get('vision_validation_pass') is True,'runtime selected generation lacks native vision validation')
    ...
    req(c.get('page_count')==2 and c.get('pages')==['📊 Current Status','🏋️ Training Center'] and c.get('hidden_entry_no_title') is True and c.get('hidden_entry_no_tabindex') is True and c.get('native_vision_visible') is True,'console proof failed')
    print(f'PHASE6_STEP4_OK primary=G2 provider=0 native_vision=PASS reverse_concept=bicycle reverse_roundtrip=1 pages=2 auth=scrypt status=REAL params={params} vision_params={vision_params} step5={step5_state}')
