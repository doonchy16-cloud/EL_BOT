...
def main():
    ...
    collapse=str(vision.get("collapse_prevention") or ""); req("direction-invariant pre-encoder concept grounding" in collapse and "concept-identity" in collapse and "direction-specific semantics introduced only by shared encoder attention" in collapse,"visual concept identity and output direction are not stage-separated")
    ...
    req("keys=[item.concept for item in selected]" in trainer,"visual content contrastive key is not concept-only"); req("keys=[(item.concept,item.direction)" not in trainer,"output direction was incorrectly reintroduced as a visual-content negative"); req("CosineAnnealingLR" in trainer and "eta_min=0.0002" in trainer,"visual adapter decay schedule missing")
