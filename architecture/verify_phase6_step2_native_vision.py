...
def main():
    ...
    req(vision.get("enabled") is True and vision.get("modality")=="native pixels","G1 visual training proof missing"); req(vision.get("truth_source")=="deterministic synthetic pixel scenes","visual positive truth is not independently deterministic")
    grounding=str(vision.get("grounding_source") or ""); req("canonical frozen Forgey ABC source embedding before encoder" in grounding and "direction-conditioned frozen decoder-ready text memory after encoder" in grounding,"visual content is not grounded at the correct representation stages")
    stages=str(vision.get("representation_stages") or ""); req("pre-encoder concept identity" in stages and "post-encoder direction-conditioned decoder memory" in stages,"visual representation-stage evidence missing")
    req(vision.get("batching_strategy")=="deterministic stratified concept-direction coverage plus seeded training-only extras","visual adapter batching is not deterministically class-balanced")
    req(int(vision.get("semantic_class_count",0))==26,"native visual semantic class cardinality changed")
    ...
    collapse=str(vision.get("collapse_prevention") or ""); req("direction-invariant pre-encoder concept grounding" in collapse and "concept-identity contrastive separation" in collapse and "post-encoder concept+direction contrastive separation" in collapse,"visual class-separation contract is incomplete")
    req(vision.get("learning_rate_schedule")=="cosine 0.003 -> 0.0002","native vision does not use the locked decaying adapter schedule"); req(float(vision.get("final_learning_rate",1.0))<float(vision.get("initial_learning_rate",0.0)),"visual adapter learning rate did not decay")
    req(float(vision.get("late_ce_loss",999))<float(vision.get("early_ce_loss",0)),"G1 visual decoder loss did not improve"); req(float(vision.get("late_alignment_loss",999))<float(vision.get("early_alignment_loss",0)),"G1 visual semantic alignment did not improve"); req(float(vision.get("late_raw_alignment_loss",999))<float(vision.get("early_raw_alignment_loss",0)),"G1 pre-encoder concept alignment did not improve"); req(float(vision.get("late_encoded_alignment_loss",999))<float(vision.get("early_encoded_alignment_loss",0)),"G1 post-encoder decoder-memory alignment did not improve"); req(float(vision.get("late_encoded_contrastive_loss",999))<float(vision.get("early_encoded_contrastive_loss",0)),"G1 post-encoder semantic class separation did not improve"); req(int(vision.get("vision_parameters",0))==report.vision_parameters,"G1 visual parameter proof differs from graph"); protected=tuple(vision.get("protected_text_probes",())); req(len(protected)>=8 and all(bool(item.get("exact")) for item in protected),"vision grounding did not preserve all protected text probes")
    ...
    guarded={"model":read(ROOT/"🧠"/"🤖"),"vision":read(ROOT/"🧠"/"👁️"),"vision_train":read(ROOT/"scripts"/"phase6-step2-train-vision.py"),"rehearsal":read(ROOT/"scripts"/"phase6-step2-rehearse-g1.py"),"vision_infer":read(ROOT/"scripts"/"phase6-vision-infer.py")}
    trainer=guarded["vision_train"]
    req("def _canonical_source" in trainer and 'encode_source("ABC_TO_EL", target.abc)' in trainer,"visual content is not grounded to canonical ABC source truth")
    req("def _direction_source" in trainer and 'encode_source("EL_TO_ABC", target.el)' in trainer,"visual output-direction text source is missing")
    req("canonical_embedding=model._embed(canonical_source)" in trainer,"pre-encoder visual content is not aligned to pre-encoder text source embeddings")
    req("raw_visual_pool=visual_source[:,1:,:].mean" in trainer,"raw pre-encoder visual concept pool missing")
    req("direction_text_pool=_semantic_text_pool(direction_memory" in trainer and "encoded_visual_pool=visual_memory[:,1:,:].mean" in trainer,"post-encoder direction-conditioned decoder-memory alignment missing")
    req("def _stratified_batch" in trainer and "selected=_stratified_batch" in trainer,"deterministic semantic-class batch coverage missing")
    req("concept_keys=[item.concept for item in selected]" in trainer,"pre-encoder contrastive keys are not concept-only")
    req("decoder_keys=[(item.concept,item.direction) for item in selected]" in trainer,"post-encoder contrastive keys do not separate concept+direction decoder classes")
    req("encoded_contrastive=_multi_positive_contrastive(encoded_visual_pool,direction_text_pool,decoder_keys)" in trainer,"post-encoder decoder-memory contrastive objective missing")
    req("CosineAnnealingLR" in trainer and "eta_min=0.0002" in trainer,"visual adapter decay schedule missing")
    rehearsal=guarded["rehearsal"]; req("torch.manual_seed(int(args.seed))" in rehearsal,"G1 rehearsal does not deterministically seed PyTorch dropout")
