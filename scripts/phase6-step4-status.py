...
def main() -> None:
    ...
    output = {
        'schema_version': 1, 'phase': 6, 'step': 4, 'available': True,
        ...
        'model': {'loadable': None, 'trainable_parameters': None, 'total_parameters': None, 'vocabulary_size': None, 'model_file_bytes': None, 'tokenizer_file_bytes': None},
    ...
    step1 = read_json(ROOT / 'data' / 'phase6-step1-data-manifest.json')
    step2 = read_json(ROOT / 'architecture' / 'phase6_step2_g0_g1_manifest.json')
    step3 = read_json(ROOT / 'architecture' / 'phase6_step3_teacher_learning_manifest.json')
    output['knowledge'] = {
        'step1_available': bool(step1), 'step2_status': step2.get('status') if step2 else None, 'step3_status': step3.get('status') if step3 else None,
        'emoji_count': (step1 or {}).get('emoji_count'), 'oewn_index_records': (step1 or {}).get('oewn_index_records'),
    }
    if step2:
    ...
    if registry:
        selected = str(registry.get('production_generation') or '')
        generations = dict(registry.get('generations') or {})
        selected_record = dict(generations.get(selected) or {})
        ...
        for generation_id, record in generations.items():
            item = dict(record); model_path = Path(str(item.get('model_path') or '')); tokenizer_path = Path(str(item.get('tokenizer_path') or ''))
        ...
        if selected_record:
            model_path = Path(str(selected_record.get('model_path') or '')); tokenizer_path = Path(str(selected_record.get('tokenizer_path') or ''))
            ...
            output['model']['model_file_bytes'] = model_path.stat().st_size if model_path.is_file() else None
            output['model']['tokenizer_file_bytes'] = tokenizer_path.stat().st_size if tokenizer_path.is_file() else None
            ...
            output['training'] = {
                ...
                'adversarial_integrity_pass': metrics.get('adversarial_integrity_pass'), 'validation_pass': metrics.get('validation_pass'),
            }
            try:
                tokenizer_module = load('_el_step4_status_tokenizer', ROOT / '📚' / '✂️')
                model_module = load('_el_step4_status_model', ROOT / '🧠' / '🤖')
                ...
                output['model'].update({'loadable': True, 'trainable_parameters': int(report.trainable_parameters), 'total_parameters': int(report.total_parameters), 'vocabulary_size': int(tokenizer.vocab_size), 'metadata': metadata})
            except Exception as error:
                output['model'].update({'loadable': False, 'load_error': type(error).__name__})
    else:
        output['available'] = False; output['reason'] = 'generation-registry-unavailable'

    evidence_dir = registry_path.parent
    teacher = read_json(evidence_dir / 'teacher-evidence.json')
    output['teacher'] = {
        'provider': (teacher or {}).get('provider') or ((teacher or {}).get('lessons') or [{}])[0].get('provider') if teacher else None,
        'model': ((teacher or {}).get('lessons') or [{}])[0].get('model') if teacher else None,
        'calls': (teacher or {}).get('provider_calls'), 'admitted': (teacher or {}).get('accepted_count'), 'rejected': (teacher or {}).get('rejected_count'),
        'provider_authored_el_truth': (teacher or {}).get('provider_authored_el_count'), 'self_output_truth': (teacher or {}).get('unverified_self_output_truth_count'),
    }
    ...
    output['training_proof'] = read_json(evidence_dir / 'g2-training-proof.json')
    output['promotion'] = read_json(evidence_dir / 'promotion-proof.json')

    try:
        diagnostics_module = load('_el_step4_status_diag', ROOT / '🧪' / '🧪')
        report = diagnostics_module.DiagnosticsEngine().run()
        output['diagnostics'] = {'available': True, 'passed': bool(report.passed), 'count': len(report.checks), 'render': report.render_el()}
    except Exception as error:
        output['diagnostics'] = {'available': False, 'passed': None, 'count': None, 'error': type(error).__name__}

    if args.validate:
        output['validation'] = {'registry_hashes': output['registry']['hashes_verified'], 'model_loadable': output['model']['loadable'], 'diagnostics_passed': output['diagnostics']['passed']}
