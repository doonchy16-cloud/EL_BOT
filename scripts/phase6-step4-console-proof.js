...
async function waitFor(fn,timeout=15000){const end=Date.now()+timeout;while(Date.now()<end){try{const v=await fn();if(v)return v;}catch(_){}await sleep(100);}throw new Error('proof timeout');}
...
await waitFor(()=>win.webContents.executeJavaScript("Boolean(document.getElementById('el-admin-console'))",true),20000);
