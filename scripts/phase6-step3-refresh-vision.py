...
def main() -> None:
    ...
    step3 = Path(args.step3_artifact).resolve(); step2_proof_path = ROOT / "data" / "phase6-step2" / "training-proof.json"
    ...
    if int(visual["provider_generated_truth_count"]) != 0 or int(visual["unverified_self_output_truth_count"]) != 0: raise RuntimeError("G2 visual replay violated truth boundary")
    ...
    if float(visual["late_loss"]) > float(visual["early_loss"]) * 0.82: raise RuntimeError(f"G2 visual adapter did not materially learn: {visual['early_loss']}->{visual['late_loss']}")

    protected = {( "ABC_TO_EL", "bicycle"): "🚲", ("EL_TO_ABC", "🚲"): "bicycle", ("ABC_TO_EL", "rocket"): "🚀", ("EL_TO_ABC", "🚀"): "rocket"}
    text_rows = []
    ...
    metadata.update({"vision_enabled": True, "vision_refresh_steps": int(args.steps), "vision_refresh_seed": int(args.seed), "vision_truth_source": visual["truth_source"], "vision_probe_exact": f"{visual['probe_exact_count']}/{visual['probe_total']}"})
    model.save_checkpoint(model_path, metadata=metadata); report = model.parameter_report()
    proof = json.loads(proof_path.read_text(encoding="utf-8")); g1_proof = json.loads(step2_proof_path.read_text(encoding="utf-8")); g1_vision = dict(g1_proof.get("vision") or {})
    if g1_vision.get("enabled") is not True or int(g1_vision.get("probe_total", 0)) <= 0 or int(g1_vision.get("probe_exact_count", -1)) != int(g1_vision.get("probe_total", 0)):
        raise RuntimeError("verified G1 native-vision baseline evidence missing")
    ...
    proof["baseline_metrics"]["vision_probe_exact"] = int(g1_vision["probe_exact_count"]); proof["baseline_metrics"]["vision_probe_total"] = int(g1_vision["probe_total"])
    proof.setdefault("candidate_metrics", {})["vision_validation_pass"] = True
    proof["candidate_metrics"]["vision_probe_exact"] = int(visual["probe_exact_count"]); proof["candidate_metrics"]["vision_probe_total"] = int(visual["probe_total"])
